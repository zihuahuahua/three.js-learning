...
export default {
  ...
  methods: {
    ...
    addGeometry() {
      ...
      const pathShape = transformSVGPathExposed(d); // //将SVG字符串转换为three.js可以识别的格式
      const extrudeSettings = {
        amount: 2, // 拉伸数量
        bevelEnabled: false // 是否使用斜角
        // curveSegments: 1, // 曲线分段数
      };

      const geometry = new THREE.ExtrudeGeometry(pathShape, extrudeSettings); // 平面拉高成几何体
  ...
  mounted() {
    ...
    buildLightSystem(this.scene); // 光源
    this.addGeometry();
    this.loop();
  }
};
